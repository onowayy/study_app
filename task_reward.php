<?php
// ==========================================================
// データベース接続（メンバーAの db_config.php を読み込む）
// ==========================================================
...
// ==========================================================
// 防衛ライン②：課題の完了処理 ＆ 資源・ステータス連動（バックエンド）
// ==========================================================
if (isset($_POST['complete_task_id'])) {
    ...
    try {
        ...
        if ($task) {
            ...
            // 難易度に応じた資源の加算量を決めるロジック
            // 例：難易度1=10、難易度2=20、難易度3=30
            $gain_materials = $difficulty * 10; 
            $gain_food = $difficulty * 10;

            // 2. 課題を「完了（is_completed = 1）」に更新する[cite: 2]
            $updateTask = $pdo->prepare("UPDATE tasks SET is_completed = 1 WHERE id = :id");
            $updateTask->execute([':id' => $task_id]);

            // 3. user_statusテーブルを更新（資源獲得 ＆ ステータス上昇）[cite: 2]
            if ($subject === '数学') {
                // 数学なら：建築資材とエサを増やし、さらに数学レベルを+1する[cite: 2]
                $updateStatus = $pdo->prepare("
                    ...
                    WHERE id = 1
                ");
                $message = "<p style='color: blue; font-weight: bold;'>【課題完了】数学Lvが1アップ！ 建築資材+{$gain_materials}、エサ+{$gain_food} 獲得！</p>";
            } else {
                // その他の教科：建築資材とエサだけを増やす[cite: 2]
                $updateStatus = $pdo->prepare("
                    UPDATE user_status 
                    SET materials = materials + :materials, 
                        food = food + :food 
...
// ==========================================================
// 【初期フェーズ】課題の新規登録処理（INSERT）
// ==========================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_task'])) {
    $title         = $_POST['title'];
    $subject       = $_POST['subject'];
    $deadline      = $_POST['deadline'];
    $required_time = (int)$_POST['required_time']; //[cite: 2]
    $difficulty    = (int)$_POST['difficulty'];    //[cite: 2]

    try {
        $sql = "INSERT INTO tasks (title, subject, deadline, required_time, difficulty, is_completed) 
                VALUES (:title, :subject, :deadline, :required_time, :difficulty, 0)"; //[cite: 2]
...
// ==========================================================
// 防衛ライン①：現在のゲームステータスと、未完了課題の取得（SELECT）
// ==========================================================
// ゲームステータスの取得（画面にリアルタイム連動を表示するため）[cite: 2]
$statusStmt = $pdo->query("SELECT * FROM user_status WHERE id = 1"); //[cite: 2]
$user_status = $statusStmt->fetch(PDO::FETCH_ASSOC);

// 未完了の課題を全件引っ張ってくる[cite: 2]
$taskStmt = $pdo->query("SELECT * FROM tasks WHERE is_completed = 0"); //[cite: 2]
$raw_tasks = $taskStmt->fetchAll(PDO::FETCH_ASSOC);

// ==========================================================
// 防衛ライン①：自動優先順位ソートロジック（PHP側で計算して並び替え）
// ==========================================================
$today = new DateTime();

foreach ($raw_tasks as &$task) {
    // 1. 期限スコア（締切が近いほど高得点）
    $deadline_date = new DateTime($task['deadline']); //[cite: 2]
    $interval = $today->diff($deadline_date);
    $days_left = (int)$interval->format('%r%a'); // 残り日数（過ぎてたらマイナス）

    if ($days_left <= 0) {
        $deadline_score = 100; // 期限当日、または超過は最優先
    } elseif ($days_left === 1) {
        $deadline_score = 50;
    } elseif ($days_left === 2) {
        $deadline_score = 30;
    ...
    // 2. 難易度スコア（難しいほど高得点）[cite: 2]
    $difficulty_score = (int)$task['difficulty'] * 10; // 10点〜30点[cite: 2]

    // 3. 所要時間スコア（時間がかかるものほど高得点）[cite: 2]
    $time_score = (int)$task['required_time'] * 0.1; // 60分なら6点[cite: 2]

    // 総合優先度スコアを合算してタスク配列にねじ込む
    $task['priority_score'] = $deadline_score + $difficulty_score + $time_score;
    $task['days_left'] = $days_left; // リマインド判定用に残り日数も保持
}
unset($task); // 参照渡しを解除

// 算出されたスコアが高い順（降順）に配列を並び替える（usort）
...
<head>
    ...
    <link rel="stylesheet" href="css/style.css"> <!--[cite: 1] -->
</head>
<body style="font-family: Arial, sans-serif; margin: 30px; line-height: 1.6;">
    ...
    <!-- ========================================================== -->
    <!-- 防衛ライン②：リアルタイム表示される現在のゲームステータス（目視確認用） -->
    <!-- ========================================================== -->
    <div style="background-color: #f0f8ff; border: 2px solid #008080; padding: 15px; margin-bottom: 20px; border-radius: 5px;">
        ...
        <p>
            ...
            <strong>エサ（food）:</strong> <?php echo $user_status['food'] ?? 0; ?> | 
            <strong>数学レベル（math_level）:</strong> Lv.<?php echo $user_status['math_level'] ?? 1; ?>[cite: 2]
        </p>
    </div>
    ...
    <div style="border: 1px solid #ccc; padding: 20px; max-width: 500px; margin-bottom: 30px; background-color: #fafafa;">
        <h2>新規課題の登録</h2>
        <form action="task_reward.php" method="POST"> <!--[cite: 1] -->
            ...
            <p>
                ...
                <input type="number" name="required_time" min="1" required placeholder="例：30" style="width: 90%;"> <!--[cite: 2] -->
            </p>
            <p>
                <label>難易度:</label><br>
                <input type="radio" id="d1" name="difficulty" value="1" checked><label Kakaku for="d1">1: 簡単</label>
            ...
            <?php else: ?>
                ...
                <?php foreach ($raw_tasks as $task): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($task['deadline'], ENT_QUOTES, 'UTF-8'); ?>[cite: 2]</td>
                        <td><?php echo htmlspecialchars($task['required_time'], ENT_QUOTES, 'UTF-8'); ?> 分[cite: 2]</td>
                        <td>レベル <?php echo htmlspecialchars($task['difficulty'], ENT_QUOTES, 'UTF-8'); ?>[cite: 2]</td>
                        <td style="color: #666; font-size: 0.9em;"><?php echo $task['priority_score']; ?> pt</td>
                        <td>
                            <!-- 防衛ライン②：完了ボタン（押すとPOSTされてデータが即座に連動） -->
                            <form action="task_reward.php" method="POST" style="margin: 0;"> <!--[cite: 1] -->
                                <input type="hidden" name="complete_task_id" value="<?php echo $task['id']; ?>"> <!--[cite: 2] -->
